...
public class UtilsTesting {
	...
	public static void main(String[] args) {
		...
		System.out.println("\nTEST 4 - Test Elite Integration to the proportional genetic breed system");
		{
			...
			s2.instantaneous_fitness_score = 6;
			s3.instantaneous_fitness_score = 5;
			s1.instantaneous_fitness_score = 4;
			rand = new Genotype(new ArrayList<>(Arrays.asList(s1, s2, s3)));
			GenotypeInnovator.createNewGenoTypeTrial3(rand, 2);
		}

		System.out.println("\nTEST 5 - Test min index of a set");
		...
		System.out.println("\nTEST 5.1 - Test min index of a set and ignore indices: 4, and 5");

		{
			ArrayList<Float> nums = new ArrayList<>(Arrays.asList(5.2f, 5f, 4f, 6f));
			System.out.println(nums);
			System.out.println("Lowest Value is at: [" + GeneticUtils.findIndexOfSmallestValue(nums, new int[] {2, 3}) + "]");
		}
